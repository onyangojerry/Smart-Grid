...
class ControlRepository:
    ...
    def _ensure_control_schema(self) -> None:
        ...
        elif not self._column_exists("edge_gateways", "provisioning_secret"):
             with self._connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("ALTER TABLE edge_gateways ADD COLUMN provisioning_secret TEXT")
                    import secrets
                    cur.execute("SELECT id FROM edge_gateways")
                    for row in cur.fetchall():
                        cur.execute("UPDATE edge_gateways SET provisioning_secret = %s WHERE id = %s", (secrets.token_urlsafe(32), row["id"]))
                    cur.execute("ALTER TABLE edge_gateways ALTER COLUMN provisioning_secret SET NOT NULL")
        if self._table_exists("edge_gateways") and not self._column_exists("edge_gateways", "metadata"):
             with self._connect() as conn:
                with conn.cursor() as cur:
                    cur.execute("ALTER TABLE edge_gateways ADD COLUMN metadata JSONB NOT NULL DEFAULT '{}'::jsonb")
    @staticmethod
